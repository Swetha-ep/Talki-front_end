// export const userAPI = `http://localhost:8000/accounts`
// export const chatAPI = `http://localhost:8000/chat`
// export const adminAPI = `http://localhost:8000/dashboard/`
// export const trainerAPI = `http://localhost:8000/trainer/`

// export const wsApiUrl = 'ws://127.0.0.1:8000';


export const userAPI = `https://talki.onrender.com/accounts`
export const chatAPI = `https://talki.onrender.com/chat`
export const adminAPI = `https://talki.onrender.com/dashboard`
export const trainerAPI = `https://talki.onrender.com/trainer`

export const paymentAPI = 'https://talki.onrender.com/payment'
export const wsApiUrl = 'wss://talki.onrender.com';